...
from configs import get_multiview_fusion_honeybee_config, FusionDataConfig
...
def parse_args():
    parser = argparse.ArgumentParser(description='MultiView-Fusion-Honeybee model training')
...
def main():
    ...
    config = get_multiview_fusion_honeybee_config()
    ...
    logger.info(f"MultiView-Fusion-Honeybee training configuration:")
    logger.info(f"   data directory: {config.data_dir}")
    logger.info(f"   output directory: {config.output_dir}")
    logger.info(f"   batch size: {config.batch_size}")
    logger.info(f"   learning rate: {config.learning_rate}")
    logger.info(f"   training epochs: {config.num_epochs}")
    logger.info(f"   Honeybee token number: {config.honeybee_num_tokens}")
    ...
    # 开始训练
    logger.info(f"starting MultiView-Fusion-Honeybee model training...")
    trainer.train(train_dataloader, val_dataloader)
    
    logger.info(f"MultiView-Fusion-Honeybee training completed!")
