...
class LEOConfig:
    ...
    output_dir: str = './checkpoints_baseline'
    ...
    def __post_init__(self):
        if self.enable_multiview_fusion:
            self.output_dir = './checkpoints_baseline_fusion'
        else:
            self.output_dir = './checkpoints_baseline'
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Baseline multimodal Vicuna training')
    ...
    logger.info("=" * 50)
    logger.info("starting baseline multimodal Vicuna training")
    logger.info("=" * 50)
    ...
    if config.enable_multiview_fusion:
        ...
        logger.info(f"  - processing方式: 每个模态独立拼接成2x2网格")
